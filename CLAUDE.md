...
## What this is
A self-hosted, phone-first food app for **two users on a home LAN**, running on a **Raspberry Pi 4 (4 GB)**. Two collections: **Recipes** (cook) and **Places** (eat out). Built in **four chunks (A→D)** — see spec §16 and the `chunk-*-prompt.md` files. You are always building exactly one chunk; do not build ahead.
...
3. **Everything local & offline-capable.** No third-party cloud for recipe data. On import, **download remote images into the local media store** — never hot-link (source pages rot).
